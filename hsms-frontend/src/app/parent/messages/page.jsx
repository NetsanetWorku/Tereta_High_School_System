...
export default function ParentMessages() {
  ...
  const [newConvForm, setNewConvForm] = useState({ teacher_id: '', subject: '', message: '' })
  ...
  const fetchTeachers = async () => {
    try {
      const res = await messageAPI.getAvailableTeachers()
      const data = Array.isArray(res.data?.data) ? res.data.data : Array.isArray(res.data) ? res.data : []
      setTeachers(data)
    } catch (e) {}
  }
  ...
  const startConversation = async (e) => {
    e.preventDefault()
    if (!newConvForm.teacher_id || !newConvForm.subject || !newConvForm.message) return
    try {
      const res = await messageAPI.startConversation(newConvForm)
      const conv = res.data?.conversation || res.data
      toast.success('Conversation started')
      setShowNewConv(false)
      setNewConvForm({ teacher_id: '', subject: '', message: '' })
  ...
  return (
    <DashboardLayout requiredRole="parent">
      <div className="flex h-screen pt-0" style={{ height: 'calc(100vh - 64px)' }}>
        {/* Sidebar */}
              ...
              {conversations.map((conv) => (
                <button
                  ...
                  className={`w-full text-left p-4 border-b border-gray-100 hover:bg-gray-50 transition ${
                    selectedConv?.id === conv.id ? 'bg-purple-50 border-l-4 border-l-purple-600' : ''
                  }`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-medium text-gray-900 text-sm truncate">
                      {conv.teacher?.user?.name || 'Teacher'}
                    </span>
          ...
          {selectedConv ? (
            <>
              ...
              <div className="bg-white border-b border-gray-200 p-4 flex justify-between items-center">
                <div>
                  <h3 className="font-semibold text-gray-900">{selectedConv.teacher?.user?.name || 'Teacher'}</h3>
              ...
              {selectedConv.status === 'open' ? (
                <form onSubmit={sendMessage} className="bg-white border-t border-gray-200 p-4 flex gap-3">
                  ...
                  <button
                    ...
                    className="px-5 py-2 bg-purple-600 text-white rounded-full text-sm hover:bg-purple-700 transition disabled:opacity-50"
                  >
                    Send
                  </button>
                </form>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-gray-500">Or start a new one to message a teacher</p>
            </div>
          )}
        </div>
      </div>
            ...
            <form onSubmit={startConversation} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewConvForm(p => ({ ...p, teacher_id: e.target.value }))}
